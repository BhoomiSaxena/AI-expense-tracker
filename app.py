...
from db import init_db, add_transactions, get_all_transactions_df


st.title("💰 AI Expense Tracker")

# Initialize DB (SQLite) sessionmaker
session_maker = init_db()


def _display_dashboard(df: pd.DataFrame):
    df = df.copy()
...
if uploaded_file:
    ...
    # Persist to DB
    try:
        add_transactions(session_maker, df)
        st.success("Transactions saved to local SQLite database.")
    except Exception as exc:
        st.error(f"Failed to save to database: {exc}")

    _display_dashboard(df)
else:
    if st.button("Load saved transactions from DB"):
        df = get_all_transactions_df(session_maker)
        if df.empty:
            st.info("No saved transactions found in the database.")
        else:
            _display_dashboard(df)
